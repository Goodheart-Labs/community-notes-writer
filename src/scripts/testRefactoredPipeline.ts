...
async function testRefactoredPipeline() {
  ...
  // Test tweet
  const testTweet =
    "The unemployment rate has reached 20% this month according to new government data.";
  const testNote =
    "According to the Bureau of Labor Statistics, the unemployment rate for December 2024 was 3.7%, not 20%. https://www.bls.gov/news.release/empsit.nr0.htm";
  ...
  try {
    ...
    const urlResult = await checkUrlValidity(
      testNote,
    ...
    const disagreementResult = await checkSubstantiveDisagreement(
      testNote,
      testTweet
    );
