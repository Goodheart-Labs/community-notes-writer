...
const softTimeout = setTimeout(() => {
  console.log(
    "[TEST] Soft timeout reached (20 minutes), stopping new processing"
  );
...
async function runTestPipeline(
  ...
  try {
    ...
    console.log(
      `[TEST ${idx + 1}] → Sarcasm score: ${sarcasmResult.score.toFixed(2)}`
    );
    ...
    console.log(
      `[TEST ${idx + 1}] → Keywords: ${keywords.keywords
        .slice(0, 3)
        .join(", ")}...`
    );
    ...
    console.log(
      `[TEST ${idx + 1}] → Characters: ${charLimitResult.characterCount}/${
        charLimitResult.limit
      } - ${charLimitResult.valid ? "PASS" : "FAIL"}`
    );

    if (!charLimitResult.valid) {
      console.log(
        `[TEST ${idx + 1}] ❌ FAILED: Note exceeds 280 character limit`
      );
      return {
        post,
        sarcasmScore: sarcasmResult.score,
    ...
    const allPassed =
      scores.url > 0.5 &&
      scores.positive > 0.5 &&
      scores.disagreement > 0.5 &&
      charLimitResult.valid;

    console.log(`[TEST ${idx + 1}] SCORES SUMMARY:`);
    console.log(
      `[TEST ${idx + 1}]   Sarcasm: ${sarcasmResult.score.toFixed(2)} ✓`
    );
    console.log(
      `[TEST ${idx + 1}]   Character Limit: ${charLimitResult.characterCount}/${
        charLimitResult.limit
      } ${charLimitResult.valid ? "✓" : "✗"}`
    );
    console.log(
      `[TEST ${idx + 1}]   URL: ${scores.url.toFixed(2)} ${
        scores.url > 0.5 ? "✓" : "✗"
      }`
    );
    console.log(
      `[TEST ${idx + 1}]   Positive: ${scores.positive.toFixed(2)} ${
        scores.positive > 0.5 ? "✓" : "✗"
      }`
    );
    console.log(
      `[TEST ${idx + 1}]   Disagreement: ${scores.disagreement.toFixed(2)} ${
        scores.disagreement > 0.5 ? "✓" : "✗"
      }`
    );
    console.log(
      `[TEST ${idx + 1}] ${
        allPassed ? "✅ ALL PASSED" : "❌ FAILED THRESHOLDS"
      }`
    );

    if (allPassed) {
      console.log(
        `[TEST ${idx + 1}] 📝 Note would be posted (but we're in test mode):`
      );
      console.log(
        `[TEST ${idx + 1}]    "${noteResult.note.substring(0, 100)}..."`
      );
    }
...
function createTestLogEntry(result: PipelineResult, branchName: string): any {
  ...
  try {
    ...
    for (const [idx, post] of posts.entries()) {
      ...
      if (shouldStopProcessing) {
        console.log(
          `[TEST] Soft timeout reached, skipping remaining ${
            posts.length - idx
          } posts`
        );
        break;
    ...
    const failedSarcasm = results.filter(
      (r) => (r.sarcasmScore || 1) <= 0.5
    ).length;
    const failedOther = results.filter(
      (r) => !r.allScoresPassed && (r.sarcasmScore || 1) > 0.5
    ).length;
