...
import { runScoringFilters, checkAllThresholds } from "../pipeline/scoringFilters";
import PQueue from "p-queue";
import { execSync } from "child_process";

// Import existing implementations
import { searchV1 } from "../pipeline/searchContextGoal";
import { writeV1 } from "../pipeline/writeNoteGoal";
import { checkV1 } from "../pipeline/checkGoal";
...
async function runRefactoredPipeline(post: any, idx: number): Promise<PipelineResult | null> {
  console.log(`[pipeline] Starting refactored pipeline for post #${idx + 1} (ID: ${post.id})`);
  ...
  try {
    ...
    console.log(`[pipeline] Sarcasm score: ${sarcasmResult.score.toFixed(2)} - ${sarcasmResult.reasoning}`);
    
    if (sarcasmResult.score <= 0.5) {
      console.log(`[pipeline] Post filtered for sarcasm (score: ${sarcasmResult.score.toFixed(2)})`);
      return {
        post,
    ...
    console.log(`[pipeline] Scores - URL: ${scores.url.toFixed(2)}, Positive: ${scores.positive.toFixed(2)}, Disagreement: ${scores.disagreement.toFixed(2)}`);
...
function createLogEntryWithScores(
  ...
  let fullResult = `SARCASM SCORE: ${result.sarcasmScore?.toFixed(2) || 'N/A'}\n\n`;
...
async function main() {
  try {
    ...
    console.log(`[main] Branch: ${currentBranch}, Submit notes: ${shouldSubmitNotes}`);
    
    if (!shouldSubmitNotes) {
      console.log(`[main] SIMULATION MODE - notes will not be submitted to X.com`);
    }
    ...
    console.log(`[main] Starting refactored pipelines for ${posts.length} posts...`);
      ...
      queue.add(async () => {
        ...
        if (result.allScoresPassed && shouldSubmitNotes) {
          try {
            ...
            console.log(`[main] Successfully submitted note for post ${post.id}:`, response);
            postedToX = true;
            submitted++;
          } catch (err: any) {
            console.error(`[main] Failed to submit note for post ${post.id}:`, err.response?.data || err);
          }
        }
    ...
    console.log(`[main] Completed - processed ${posts.length} posts, submitted ${submitted} notes`);
