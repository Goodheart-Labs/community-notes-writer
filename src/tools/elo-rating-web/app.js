...
class CommunityNotesComparison {
    ...
    initializeEventListeners() {
        ...
        const leftSlider = document.getElementById('leftRating');
        const leftInput = document.getElementById('leftRatingInput');
        const rightSlider = document.getElementById('rightRating');
        const rightInput = document.getElementById('rightRatingInput');
        leftSlider?.addEventListener('input', () => {
            leftInput.value = (parseFloat(leftSlider.value) / 10).toFixed(1);
        });
        leftInput?.addEventListener('input', () => {
            leftSlider.value = (parseFloat(leftInput.value) * 10).toString();
        });
        rightSlider?.addEventListener('input', () => {
            rightInput.value = (parseFloat(rightSlider.value) / 10).toFixed(1);
        });
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (this.awaitingRating)
                return;
            if (!this.comparisonPairs.length || this.currentPairIndex >= this.comparisonPairs.length)
                return;
      ...
      `;
        ...
        try {
            ...
            if (cachedTweets) {
                ...
                this.airtableClient.onProgress = (message) => showProgress(message);
        ...
        for (const tweet of this.tweets) {
            ...
            for (let i = 0; i < tweet.notes.length; i++) {
                for (let j = i + 1; j < tweet.notes.length; j++) {
                    // Skip if same bot
                    if (tweet.notes[i].botName === tweet.notes[j].botName)
                        continue;
    ...
    calculateCommunityNoteLength(text) {
    ...
    displayNoteWithLinks(elementId, noteText, status) {
    ...
    showRatingInterface() {
        ...
        document.getElementById('leftRating').value = '50';
        document.getElementById('leftRatingInput').value = '5.0';
        document.getElementById('rightRating').value = '50';
        document.getElementById('rightRatingInput').value = '5.0';
        ...
        if (pair.leftNote.wouldNathanPost !== undefined) {
            ...
            document.getElementById('leftRating').value = (leftRating * 10).toString();
            document.getElementById('leftRatingInput').value = leftRating.toFixed(1);
        }
    ...
    async submitRatings() {
        if (!this.currentComparison || !this.airtableClient)
            return;
        const pair = this.comparisonPairs[this.currentPairIndex];
        const leftRating = parseFloat(document.getElementById('leftRatingInput').value);
        const rightRating = parseFloat(document.getElementById('rightRatingInput').value);
